...
class SecurityTests(TestCase):
    ...
    def test_admin_dashboard_non_staff_redirect(self):
        ...
        response = self.client.get(reverse('orders:admin_dashboard'))
        self.assertEqual(response.status_code, 302)
        self.assertIn(reverse('core:index'), response.url)
    ...
    def test_delete_order_csrf_protection(self):
        ...
        response = self.client.post(reverse('orders:delete_order', args=[self.order.id]), {'reason': 'test'})
    ...
    def test_sales_report_staff_redirect(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertIn(reverse('orders:admin_dashboard'), response.url)
